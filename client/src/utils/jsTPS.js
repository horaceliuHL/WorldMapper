...
export class ReorderTasks_Transaction extends jsTPS_Transaction {
  ...
  async undoTransaction() {
    const {data} = await this.undoCallback({ variables: {_id: this.listID, list: this.listItems }});
    return data;
  }
}
...
export class AddRegion_Transaction extends jsTPS_Transaction {
    ...
    async doTransaction(){
        const { data } = await this.addfunc({ variables: { region: this.item } })
        this.itemID = data.addRegion
        return data;
    }
    async undoTransaction(){
        const {data} = await this.delfunc({ variables: {_id: this.itemID} })
        this.item = data.deleteRegion
